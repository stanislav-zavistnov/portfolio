...
function getStudentItem(studentObj) {
  ...
  const fullName = studentObj.surname + ' ' + studentObj.name + ' ' + studentObj.middleName;
  const yearOfIssue = Number(studentObj.yearStart) + 4;
  studentsTableList.classList.add('students-list');
  studentsTable.append(studentsTableList);
  addElemInTable(fullName, studentsTableList);
  addElemInTable((studentObj.dateOfBirth + ' ' + getActuallyStudentYearsOld(studentObj.dateOfBirth)), studentsTableList);
  addElemInTable((studentObj.yearStart + '-' + String(yearOfIssue) + ' ' + getActuallyStatusOfCource(studentObj.yearStart)), studentsTableList);
...
function addElemInTable(text, studentsTableList) {
...
function getActuallyStatusOfCource(yearStart) {
  let result;
  let returnResult;
  const date = new Date();
  const actuallyYear = String(date.getFullYear());
  ...
  const monthStart = '09';
  let course = Number(actuallyYear) - Number(yearStart);
  if (course < 1) {
    result = '1 Курс';
...
function renderStudentsTable(studentsArray) {
  for (let i = 0; i < studentsArray.length; i++) {
    ...
    studentsArray[i].newDate = convertDate(studentsArray[i].dateOfBirth);
  }
}
...
function createDataArray() {
  ...
  const studentObject = {
    ...
    dateOfBirth: finalBirthsdayDate,
    yearStart: getYearStart,
    faculty: finalFaculty,
  }
  // pushing object in global array
...
// function createCopyAnyArray (anyArray) {
//   let copyArray = [];
//   for (i = 0; i < anyArray.length; i++) {
//     copyArray.push(anyArray[i]);
//   }
//   return copyArray;
// }
...
yearsOfStudyColumn.addEventListener('click', () => {
  let result;
  if (a.length === 0) {
    result = sortStudents(studentsList, 'yearStart');
    a.push(1);
  } else if (a.length === 1) {
    result = sortStudents(studentsList, 'yearStart', true);
    a.splice(0, 1);
  }
  clearTable();
  renderStudentsTable(result);
...
function filterWithInputs() {
  ...
  if (finalMiddlename !== '') {
    newArr = filter(newArr, 'middleName', finalMiddlename);
  }
  if (finalFaculty !== '') {
    newArr = filter(newArr, 'faculty', finalFaculty);
  }
  if (searhByYearStartValue !== '') {
    newArr = filter(newArr, 'yearStart', searhByYearStartValue);
  }
  if (searhByYearFinishValue !== '') {
    newArr = filter(newArr, 'yearStart', searhByYearFinishValue - 4);
  }
  return newArr;
}
